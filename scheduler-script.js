...
/* ========= Term-specific enrolled + waitlist state ========= */
const termPlans = {
    "Fall 2025": {
        enrolled: ["CSI 2300", "MTH 1554", "PHY 1510", "WRT 1060"],
        waitlist: new Map()
    },
    "Winter 2026": {
        enrolled: [],            // empty to start; user can build a Winter plan
        waitlist: new Map()
    },
    "Summer 2025": {
        enrolled: [],            // empty to start
        waitlist: new Map()
    }
};

let currentTerm = "Fall 2025";

function getEnrolled() {
    return termPlans[currentTerm].enrolled;
}

function getWaitlist() {
    return termPlans[currentTerm].waitlist;
}
...
function renderKpis() {
    const total = getEnrolled().reduce((sum, code) => sum + (courseCatalog[code]?.credits || 0), 0);
...
function renderClassGrid() {
    ...
    const enrolledNow = getEnrolled();
    const waitlistNow = getWaitlist();

    enrolledNow.forEach(code => {
        ...
        const seatsBadge = c.seats > 0
            ...
            : (waitlistNow.has(code)
                ? `<span class="badge wait">Waitlist #${waitlistNow.get(code)}</span>`
...
function renderScheduleEvents() {
    ...
    const map = new Map();
    enrolledNow.forEach(code => { const c = courseCatalog[code]; if (c) map.set(code, c); });
...
function renderSuggestions() {
    ...
    degreeProgress.unmet.forEach(req => {
        ...
        if (!c || enrolledNow.includes(req)) return;
        if (c.seats > 0) {
            const item = document.createElement("div");
            item.className = "list-item";
            item.innerHTML = `
    ...
    enrolledNow.forEach(code => {
        const c = courseCatalog[code];
        if (!c) return;
        if (c.seats === 0 && waitlistNow.has(code)) {
            const pos = waitlistNow.get(code);
            const item = document.createElement("div");
            item.className = "list-item";
            item.innerHTML = `
        <div>
...
function addCourse(code) {
    ...
    if (enrolledNow.includes(code)) { toast(`${code} already in plan`); return; }
    ...
    if (c.seats > 0) {
        c.seats -= 1;
        enrolledNow.push(code);
        toast(`Added ${code} • ${c.seats} seats left`);
        pushNotification(`Added ${code} to your ${currentTerm} plan.`);
        renderAll();
    } else {
        // add to waitlist
        const position = (waitlistNow.get(code) || 0) + 1;
        waitlistNow.set(code, position);
        toast(`${code} is full — joined waitlist (#${position})`);
        pushNotification(`Waitlist update (${currentTerm}): ${code} → position #${position}`);
        renderAll();
    }
}
...
function dropCourse(code) {
    const enrolledNow = getEnrolled();
    const idx = enrolledNow.indexOf(code);
    if (idx === -1) { toast(`${code} is not in plan`); return; }
    enrolledNow.splice(idx, 1);
    // free seat
    const c = courseCatalog[code];
    if (c) c.seats += 1;
    toast(`Dropped ${code} • ${c ? c.seats : 0} seats open`);
    pushNotification(`Dropped ${code} from your ${currentTerm} plan.`);
    renderAll();
}
...
function simulateSeatOpen(code, delayMs=8000) {
    setTimeout(() => {
        ...
        // apply to the current term's waitlist only
        const waitlistNow = getWaitlist();
        c.seats += 1;
        const currentPos = waitlistNow.get(code);
        if (currentPos && currentPos > 0) {
            waitlistNow.set(code, currentPos-1);
            pushNotification(`Seat opened for ${code} in ${currentTerm}! Your waitlist is now #${currentPos-1}.`);
...
if (termSelect) {
    ...
    termSelect.addEventListener("change", () => {
        ...
        toast(`Switched to ${currentTerm}`);
        pushNotification(`Switched view to ${currentTerm} plan.`);
        renderAll();
    });
}
...
setTimeout(() => {
    pushNotification(`Reminder: Registration window opens soon. Review your ${currentTerm} plan.`);
